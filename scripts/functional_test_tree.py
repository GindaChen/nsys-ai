...
async def run_tree_tests():
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        check(
            "Initial rows = 5 (2 NVTX + 3 kernels, all expanded)",
            dt.row_count == 5,
            f"got {dt.row_count}",
        )
        ...
        check("C: collapse all → shows only roots", after_collapse == 2, f"got {after_collapse}")

        await pilot.press("E")  # expand all
        await pilot.pause()
        check("E: expand all → 5 rows again", dt.row_count == 5, f"got {dt.row_count}")
        ...
        await pilot.pause()
        check("c: collapse single node", dt.row_count < 5, f"got {dt.row_count}")

        await pilot.press("e")
        await pilot.pause()
        check("e: expand single node back", dt.row_count == 5, f"got {dt.row_count}")

        # ── Depth filter (number keys) ─────────────────────────────────────
        await pilot.press("1")
        await pilot.pause()
        check("1: depth-1 shows only top-level (2 NVTX)", dt.row_count == 2, f"got {dt.row_count}")

        await pilot.press("2")
        await pilot.pause()
        check("2: depth-2 shows NVTX + kernels", dt.row_count > 2, f"got {dt.row_count}")

        await pilot.press("0")
        await pilot.pause()
        check("0: unlimited depth restores all", app.max_depth == -1, f"max_depth={app.max_depth}")
        ...
        await pilot.pause()
        check(
            "v: toggle view mode tree→linear",
            app.view_mode != initial_mode,
            f"view_mode={app.view_mode}",
        )

        await pilot.press("v")
        await pilot.pause()
        check("v: toggle back", app.view_mode == initial_mode, f"view_mode={app.view_mode}")
        ...
        await pilot.pause()
        check(
            "/ filter 'mm': only mm matches visible",
        ...
        await pilot.press("n")
        await pilot.pause()
        check("n: clear filter → 5 rows", dt.row_count == 5, f"rows={dt.row_count}")
        ...
        check(
            "d: toggle demangled",
            app.show_demangled != show_before,
        ...
        await pilot.press("S")
        await pilot.pause()
        check("S: save bookmark", len(app._bookmarks) == 1, f"bookmarks={len(app._bookmarks)}")

        await pilot.press("p")
        await pilot.pause()
        check(
            "p: toggle bookmarks panel",
        ...
        await pilot.pause()
        check("scroll_to_kernel first occurrence", dt.cursor_row > 0, f"cursor_row={dt.cursor_row}")

        app.scroll_to_kernel("aten::mm", 2)
        await pilot.pause()
        row_after = dt.cursor_row
        check(
            "scroll_to_kernel second occurrence",
        ...
        await pilot.pause()
        check("+: min duration increases", app.min_dur_us > old_dur, f"{old_dur}→{app.min_dur_us}")

        old_dur2 = app.min_dur_us
        await pilot.press("minus")  # Binding uses "minus,underscore"
        await pilot.pause()
        check(
            "-: min duration decreases", app.min_dur_us <= old_dur2, f"{old_dur2}→{app.min_dur_us}"
        )
...
async def run_timeline_tests():
    ...
    async with app.run_test(size=(140, 40)) as pilot:
        ...
        check("Timeline: two streams found", len(app._streams) == 2, f"streams={app._streams}")
        check(
            "Timeline: cursor at time_start",
            app.cursor_ns == app._time_start,
        ...
        await pilot.pause()
        check(
            "→: pan right moves cursor",
            app.cursor_ns > initial_cursor,
        ...
        await pilot.pause()
        check(
            "Home: jump to trace start", app.cursor_ns == app._time_start, f"cursor={app.cursor_ns}"
        )

        await pilot.press("end")
        await pilot.pause()
        check("End: jump to trace end", app.cursor_ns == app._time_end, f"cursor={app.cursor_ns}")
        ...
        await pilot.pause()
        check(
            "Shift+→: page-right bigger than single step",
        ...
        await pilot.pause()
        check(
            "+/=: zoom in decreases ns/col",
            app.ns_per_col < initial_npc,
        ...
        await pilot.pause()
        check(
            "-: zoom out increases ns/col",
            app.ns_per_col > app.ns_per_col - 1,
        ...
        check(
            "↓: select next stream",
            app.selected_stream_idx > old_si,
        ...
        await pilot.pause()
        check(
            "Tab: snap to next kernel", app.cursor_ns > app._time_start, f"cursor={app.cursor_ns}"
        )
        ...
        await pilot.pause()
        check("B: save bookmark", len(app._bookmarks) == 1, f"bookmarks={len(app._bookmarks)}")
        ...
        check(
            "[: range start set",
            app._range_start_ns is not None,
        ...
        await pilot.pause()
        check(
            "]: range end auto-saves bookmark",
            len(app._bookmarks) >= 1,
        ...
        await pilot.press("enter")
        await pilot.pause()
        check("/ filter 'mm': applied", app.filter_text == "mm", f"filter='{app.filter_text}'")

        await pilot.press("n")
        await pilot.pause()
        check("n: clear filter", app.filter_text == "", f"filter='{app.filter_text}'")
        ...
        await pilot.pause()
        check(
            "scroll_to_kernel: cursor moved to kernel start",
        ...
        await pilot.pause()
        check(
            "zoom_to_time_range: cursor updated",
            app.cursor_ns == 10_000_000,
            f"cursor={app.cursor_ns}",
        )
